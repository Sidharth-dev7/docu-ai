...
**Entry point:** `index.js` — Slack Bolt app. Listens to all messages, filters to `SLACK_RELEASES_CHANNEL`, calls `interpretAnnouncement` (Claude), then delegates to `handleRelease`.
...
7. Create Jira task with issue type `Asset` (non-fatal)
8. Email skipped (disabled — SMTP setup pending)
9. Post Slack notification to `SLACK_NOTIFICATIONS_CHANNEL` (always last — fallback for all failures)
...
- `interpretAnnouncement` — identifies product/version/affected pages from Slack text. Uses known page names from `products.json` so Claude picks exact names rather than guessing paths.
...
- Claude response JSON wrapping: `interpretAnnouncement` strips ` ```json ``` ` fences before parsing
